...
export class CryptoService {
  static async getCryptoPrice(crypto: string, currency: string): Promise<CryptoPrice | null> {
    try {
      const cryptoId = CRYPTO_MAP[crypto.toLowerCase()];
      const currencyId = CURRENCY_MAP[currency.toLowerCase()];

      console.log('Запрос данных для:', { crypto, currency, cryptoId, currencyId });
      ...
      console.log('URL запроса:', tickerUrl);
  ...
  static async getCryptoPriceAlternative(crypto: string, currency: string): Promise<CryptoPrice | null> {
    try {
      ...
      const response = await axios.get(`${COINCAP_API_URL}/assets/${cryptoId}`, {
        timeout: 10000,
      });
